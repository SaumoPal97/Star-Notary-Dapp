...
it("can add the star name and star symbol properly", async () => {
  ...
  let instance = await StarNotary.deployed();
  let starId = 6;
  await instance.createStar("Saumo6", starId, { from: accounts[0] });

  assert.equal(await instance.tokenIdToStarInfo.call(starId), "Saumo6");
...
it("lets 2 users exchange stars", async () => {
  ...
  let starId1 = 7;
  let starId2 = 8;
  await instance.createStar("Saumo7", starId1, { from: accounts[0] });
  await instance.createStar("Saumo8", starId2, { from: accounts[1] });
...
it("lets a user transfer a star", async () => {
  ...
  let instance = await StarNotary.deployed();
  let starId = 9;
  await instance.createStar("Saumo9", starId, { from: accounts[0] });
...
it("lookUptokenIdToStarInfo test", async () => {
  ...
  let instance = await StarNotary.deployed();
  let starId = 10;
  await instance.createStar("Saumo10", starId, { from: accounts[0] });

  assert.equal(await instance.lookUptokenIdToStarInfo(starId), "Saumo10");
});
